...
export function CommandPalette({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const runDemo = useStore((s) => s.runDemoCascade);
  ...
  const items: Item[] = useMemo(() => {
    ...
    const actions: Item[] = [
      {
        label: "Run cascade demo",
        group: "actions",
        icon: <LightningIcon size={14} className="text-saffron-600" />,
        onSelect: () => {
          runDemo();
          onOpenChange(false);
        },
        keywords: "simulate cancel"
      },
      {
  ...
  }, [customers, slots, router, onOpenChange, runDemo, resetDemo]);
